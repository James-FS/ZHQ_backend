...
type Message struct {
	BaseModel
	MessageID    string `json:"message_id" gorm:"type:varchar(36);uniqueIndex;not null comment:消息ID"` // UUID
	...
	SessionID    string `json:"session_id" gorm:"type:varchar(36);index;not null comment:对话ID"`       // 会话ID（支持群组）
